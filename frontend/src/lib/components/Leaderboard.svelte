...
<section class="grid divide-y rounded bg-neutral-800/60 px-1 text-[11px]">
	...
	{#each newList as driver}
		...
		>
			...
			>
				{timingData.Lines[driver].LastLapTime.Value}
			</p>
			...
			<div class="flex divide-x text-[9px] text-neutral-300/70">
				...
				>
					{timingData.Lines[driver].Sectors[0].Value}
				</div>
				<div
					class="border-neutral-700 px-1"
					class:text-green-500={timingData.Lines[driver].Sectors[1].PersonalFastest}
					class:bg-purple-500={timingData.Lines[driver].Sectors[1].OverallFastest}
				>
					{timingData.Lines[driver].Sectors[1].Value}
				</div>
				<div
					class="border-neutral-700 pl-1"
					class:text-green-500={timingData.Lines[driver].Sectors[2].PersonalFastest}
					class:bg-purple-500={timingData.Lines[driver].Sectors[2].OverallFastest}
				>
					{timingData.Lines[driver].Sectors[2].Value}
				</div>
			</div>
			...
			<div class="flex w-12 items-center justify-end gap-1 font-light text-neutral-300">
				...
				>
					<div
						class={`bg-${tireColor(tyreStintSeries.Stints[driver][tyreStintSeries.Stints[driver].length - 1].Compound.toLowerCase())} h-1 w-1 rounded bg-red-500`}
					></div>
				</div>
