...
const Map = ({ sessionInfo }: { sessionInfo: SessionGp }) => {
  ...
  useLayoutEffect(() => {
    ...
    const closestPosition = driverPositoins.flat().reduce((acc, current) => {
    ...
    >
      <div className="relative mx-auto w-full max-w-fit">
        {circuitPoints && (
          <>
            <canvas
              className="mx-auto max-h-[calc(100dvh-136px)] w-full"
              ref={circuitRef}
            ></canvas>
            <canvas
              className="absolute top-0 mx-auto max-h-[calc(100dvh-136px)] w-full"
