...
const CreateCircuit = () => {
  ...
  const [selectedCircuit, setSelectedCircuit] = useState<CircuitList>();
  ...
  useEffect(() => {
    if (circuitList) {
      setSelectedCircuit(circuitList[0]);
    }
  }, [circuitList]);
    ...
    <>
      <div className="flex flex-wrap items-center gap-2">
          ...
          >
            {circuitList && (
              <>
                <select
                  id="circuit"
                  value={selectedCircuit?.circuitKey}
                  onChange={(e) => {
                    const key = e.target.value;
                    setSelectedCircuit(
                      circuitList.find((c) => c.circuitKey === parseInt(key)),
                    );
                  }}
                ...
                >
                  {circuitList.map((circuit) => (
                    <option key={circuit.circuitKey} value={circuit.circuitKey}>
                      {circuit.name}
                    </option>
                  ))}
                </select>
        ...
        <div className="flex items-center gap-2 rounded-md bg-neutral-800 px-1.5 py-1.5">
          ...
          <div
            className={`${(!circuitList || circuitListLoading) && "animate-pulse rounded bg-neutral-600 "} relative h-7`}
          >
            {circuitList && (
              <>
                <select
                  ...
                  className="block w-full appearance-none rounded border-2 border-neutral-700 bg-neutral-700 px-2  pr-8 font-sans tracking-wider outline-none focus:border-neutral-500 "
                >
                  {selectedCircuit?.sessions.map((session) => (
